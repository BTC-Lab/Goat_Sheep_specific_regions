...
cat sheep_uniq.merged.bed | awk '{print $1, $2, $3, $3 - $2}' | sed -e 's/ /\t/g' | sort -k4 -n -r | grep -v 'NW' | awk '{print $1":"$2"-"$3}' > sheep_specific_regions_full.bed

## Calculate coverage for 1000 regions
for region in `cat sheep_specific_regions_full.bed | head -1000` 
do 
	total=0
...
cat goat_uniq.merged.bed | awk '{print $1, $2, $3, $3 - $2}' | sed -e 's/ /\t/g' | sort -k4 -n -r | grep -v 'NW' | awk '{print $1":"$2"-"$3}' > goat_specific_regions_full.bed

## Calculate coverage for 1000 regions
for region in `cat goat_specific_regions_full.bed | head -1000` 
do 
	total=0
